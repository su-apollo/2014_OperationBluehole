...
BOOL Elin::VertexConstruction(FBX_MESH_NODE& fbxNode, MESH_NODE& meshNode)
{
...
BOOL Elin::MaterialConstruction()
{
...
BOOL Elin::CreateMeshTexture(MESH_NODE& meshNode)
{
...
BOOL Elin::CreateVertexBuffer(ID3D11Buffer** pBuffer, void* pVertices, uint32_t stride, uint32_t vertexCount)
{
...
BOOL Elin::CreateIndexBuffer(ID3D11Buffer** pBuffer, void* pIndices, uint32_t indexCount)
{
	if (indexCount == 0)
		return E_FAIL;
...
void Elin::RenderNode(MESH_NODE& node)
{
	...
	UINT stride = sizeof(ELIN_VERTEX);
	UINT offset = 0;
